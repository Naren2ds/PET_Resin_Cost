...
PIPELINE_NAME = "Pricing_Pref_ABI_RD_Men_4_26_Cliente"
SOURCE_FILE = REPO_ROOT / "data" / "raw" / "suppliers" / "Pricing_Pref_ABI_RD_Men_4_26_Cliente.xlsx"
EXTRACT_SCRIPT = REPO_ROOT / "pipelines" / "suppliers" / "legacy_scripts" / "extract_pricing_pref_rd_inputs.py"
MAPPING_SCRIPT = REPO_ROOT / "pipelines" / "suppliers" / "legacy_scripts" / "data_mapping_column_rd.py"
...
EXTRACTED_FILE = EXTRACTION_DIR / "Pricing_Pref_ABI_RD_Men_4_26_Cliente_inputs_final.xlsx"
STANDARDIZED_FILE = STANDARDIZED_DIR / "Pricing_Pref_ABI_RD_Men_4_26_Cliente_actual_standardized.xlsx"
TLC_VALIDATION_FILE = CALCULATION_DIR / "rd_tlc_calculation_validation.xlsx"
FORECAST_TEMPLATE_FILE = FORECAST_DIR / "rd_forecast_inputs_template.xlsx"
FORECAST_ESTIMATION_FILE = FORECAST_DIR / "rd_2026_forward_estimation.xlsx"
FRONT_END_ACTUAL_XLSX = FRONT_END_DIR / "Pricing_Pref_ABI_RD_Men_4_26_Cliente_front_end_standardized.xlsx"
FRONT_END_ACTUAL_CSV = FRONT_END_DIR / "Pricing_Pref_ABI_RD_Men_4_26_Cliente_front_end_standardized.csv"
FRONT_END_ACTUAL_FORECAST_XLSX = (
    FRONT_END_DIR / "Pricing_Pref_ABI_RD_Men_4_26_Cliente_actual_forecast_front_end_standardized.xlsx"
)
FRONT_END_ACTUAL_FORECAST_CSV = (
    FRONT_END_DIR / "Pricing_Pref_ABI_RD_Men_4_26_Cliente_actual_forecast_front_end_standardized.csv"
)
...
SOURCE_WORKBOOK_FORMULA = "DDP Price = FOB Price + Ocean Freight + Import Clearance"
...
CALCULATION_REQUIRED_ROWS = {9, 10, 11, 12}
FREIGHT_BASE_BY_COUNTRY = {
    "El Salvador": 195.0,
    "Peru": 161.0,
}
    ...
    try:
        ...
        for sheet_name in ("Pricing formula_Updated", "Pricing formula overview"):
            ...
            for row in worksheet.iter_rows(min_row=3, values_only=True):
                ...
                metadata["formula_overview_destination_country"] = current_country
...
def read_source_inputs(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "countries": {
                "El Salvador": {
                    "freight_index_name": clean_text(value_ws["L5"].value),
                    "freight_source_period_label": clean_text(value_ws["L7"].value),
                    "freight_driver_value": maybe_float(value_ws["M7"].value),
                    "freight_driver_formula": formula_ws["M7"].value,
                    "freight_formula_cell": "H10",
                    "ddp_formula_cell": "H12",
                },
                "Peru": {
                    "freight_index_name": clean_text(value_ws["L9"].value),
                    "freight_source_period_label": clean_text(value_ws["L11"].value),
                    "freight_driver_value": maybe_float(value_ws["M11"].value),
                    "freight_driver_formula": formula_ws["M11"].value,
                    "freight_formula_cell": "I10",
                    "ddp_formula_cell": "I12",
                },
            },
        }
    finally:
...
def rows_by_period_country(rows: list[dict[str, Any]]) -> dict[tuple[str, str], list[dict[str, Any]]]:
    grouped: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for row in rows:
        grouped.setdefault((str(row["time_period"]), str(row["country"])), []).append(row)
    return grouped
...
def expected_freight(country: str, freight_driver: float | None, actual_freight: float) -> float:
    base = FREIGHT_BASE_BY_COUNTRY.get(country, actual_freight)
...
def validation_rows(
    ...
    grouped = rows_by_period_country(final_rows)
    rows: list[dict[str, Any]] = []
    ...
    for (period, country), country_rows in sorted(
        grouped.items(), key=lambda item: (parse_period(item[1][0]), item[0][1])
    ):
        fob_row = row_for_metric_row(country_rows, 9)
        freight_row = row_for_metric_row(country_rows, 10)
        clearance_row = row_for_metric_row(country_rows, 11)
        ddp_row = row_for_metric_row(country_rows, 12)
        if not all([fob_row, freight_row, clearance_row, ddp_row]):
            continue

        freight_info = source_inputs["countries"].get(country, {})
        ...
        calculated_ddp = fob + ocean_freight + import_clearance
        formula_difference = calculated_ddp - source_ddp
        freight_driver = maybe_float(freight_info.get("freight_driver_value"))
        calculated_freight = expected_freight(country, freight_driver, ocean_freight)
        ...
        rows.append(
            {
                ...
                "freight_index_name": freight_info.get("freight_index_name"),
                "freight_source_period_label": freight_info.get("freight_source_period_label"),
                "freight_driver_value": freight_driver,
                ...
                "freight_validation_status": (
                    "match" if abs(freight_difference) <= 0.000001 else "check"
                ),
...
def latest_rows_by_country(grouped: dict[tuple[str, str], list[dict[str, Any]]]) -> dict[str, list[dict[str, Any]]]:
    latest: dict[str, list[dict[str, Any]]] = {}
    for (_period, country), rows in grouped.items():
        if country not in latest or parse_period(rows[0]) > parse_period(latest[country][0]):
            latest[country] = rows
    return latest
...
def forecast_estimate_rows(
    latest_by_country: dict[str, list[dict[str, Any]]],
    index_reference: dict[tuple[str, int, int], dict[str, Any]],
    index_lag_months: int,
) -> list[dict[str, Any]]:
    latest_period = max(parse_period(rows[0]) for rows in latest_by_country.values())
    rows: list[dict[str, Any]] = []
    for period in forecast_months(latest_period):
        ...
        for country, latest_rows in sorted(latest_by_country.items()):
            ocean_freight = value_for_row(latest_rows, 10)
            import_clearance = value_for_row(latest_rows, 11)
            import_tax_rate = 0.0
            ...
            rows.append(
                {
                    ...
                    "tlc_formula": TLC_FORMULA_DISPLAY,
                    "notes": "Ocean freight, import tax, and import clearance are carried from latest actual because no forward Drewry table is available.",
                }
            )
    return rows
        ...
        rows.append(
            {
                ...
                "Destination Country": row.get("country"),
    ...
    return [
        {
            ...
            "ddp_price_forecast": row["ddp_price_forecast"],
            ...
            rows.append(
                {
                    ...
                    "Supplier Name": metadata["supplier"],
                    "Destination Country": estimate["destination_country"],
...
def write_outputs(
    ...
    run_metadata = {
        ...
        "formula_overview_destination_country": metadata.get("formula_overview_destination_country"),
        "forecast_resin_index_type": INDEX_SERIES,
        "index_lag_months": index_lag_months,
        "source_workbook_formula": SOURCE_WORKBOOK_FORMULA,
        ...
        "forecast_months": len({row["time_period"] for row in estimates}),
        "forecast_estimate_rows": len(estimates),
    ...
    write_workbook(
        ...
        [
            ("rd_2026_estimate", estimate_headers, estimates),
    ...
    summary = {
        **run_metadata,
        ...
        "forecast_periods": sorted({row["time_period"] for row in estimates}, key=lambda label: datetime.strptime(label, "%B %Y")),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the RD supplier extraction, validation, forecast, and front-end pipeline."
    )
...
def main() -> None:
    ...
    metadata["formula_overview_resin_index_type"] = metadata.get("formula_overview_resin_index_type")
    ...
    grouped = rows_by_period_country(final_rows)
    latest_by_country = latest_rows_by_country(grouped)
    ...
    estimates = forecast_estimate_rows(latest_by_country, index_reference, index_lag_months)
    ...
    print(f"countries={'; '.join(sorted(latest_by_country))}")
    ...
    print(f"forecast_estimate_rows={len(estimates)}")
