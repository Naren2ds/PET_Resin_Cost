...
def _component_amounts(entry: dict, gap_abs: float | None = None) -> list[tuple[str, float]]:
    ...
    # Driver ranking rule:
    # - If Market - Supplier delta is positive (supplier below market), surface the
    #   smallest component values first.
    # - If delta is negative (supplier above market), surface the largest values first.
    # - If delta is unavailable/zero, keep the previous largest-first behavior.
    if gap_abs is not None and gap_abs > 0:
        return sorted(amounts, key=lambda item: item[1])
    return sorted(amounts, key=lambda item: item[1], reverse=True)
...
def build_procurement_intelligence(
    ...
    for all_entries, entry in snapshot_entries:
        ...
        components = _component_amounts(entry, gap_abs=gap_abs)
