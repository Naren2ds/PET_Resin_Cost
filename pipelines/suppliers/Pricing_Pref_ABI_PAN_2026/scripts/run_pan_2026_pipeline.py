"""
Pipeline: Panama 2026 (all months)
Reads each monthly file from data/raw/suppliers/PAN/2026/
Extracts from the Inputs sheet:
  - FOB Price (H8)
  - Ocean Freight (H9)
  - Import Clearance (H10)
  - DDP Price (H11)
Outputs standardized rows with Destination Country = "Panama".
"""
...
def extract_month(path: Path) -> list[dict[str, Any]]:
    """Extract standardized rows from a single monthly Panama file."""
    ...
    fob = ws.cell(8, 8).value    # H8 - Promedio FOB
    freight = ws.cell(9, 8).value  # H9 - Flete maritimo
    clearance = ws.cell(10, 8).value  # H10 - Nacionalizacion
    ddp = ws.cell(11, 8).value   # H11 - Precio DDP
...
def main() -> None:
    ...
    if old_count > 0:
        ...
        print(f"  Removed {old_count} old Panama rows")

    new_df = pd.DataFrame(all_rows, columns=FRONT_END_HEADERS)
    print(f"  New Panama rows: {len(new_df)}")
