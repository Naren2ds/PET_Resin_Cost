...
def _supplier_data_version() -> tuple[float | None, float | None]:
...
@lru_cache(maxsize=4)
def _read_data_model_rows_cached(
...
def read_data_model_rows() -> list[dict[str, Any]]:
    return _read_data_model_rows_cached(_supplier_data_version())
...
@lru_cache(maxsize=1)
def _build_vendor_breakdown_entries() -> tuple[dict[str, Any], ...]:
...
def build_vendor_breakdowns(
    source_countries: tuple[str, ...],
    destination: str | None = None,
) -> list[dict[str, Any]]:
    """Expand cached supplier entries for the requested MR source countries.

    The expensive workbook grouping is built once. API callers can limit the
    response to one destination without rebuilding the complete timeline.
    """
    selected_destination = clean_text(destination)
    sources = tuple(source_countries) or ("China",)
    expanded: list[dict[str, Any]] = []
    for entry in _build_vendor_breakdown_entries():
        if selected_destination and clean_text(entry.get("destination")) != selected_destination:
            continue
