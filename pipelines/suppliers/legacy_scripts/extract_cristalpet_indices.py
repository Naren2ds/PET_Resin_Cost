...
DEFAULT_FILE_PATTERN = "*Cristalpet*.xlsx"
DEFAULT_OUTPUT_DIR = Path("data") / "archive" / "legacy_output"
DEFAULT_SHEET = "\u00cdndices 2025-26-27"
DEFAULT_TLC_SHEET = "Pre\u00e7os Cristalpet"
DEFAULT_RANGES = ("B2:N8", "E21:O22")
DEFAULT_RAW_CSV_NAME = "cristalpet_indices_raw.csv"
DEFAULT_LONG_CSV_NAME = "cristalpet_indices_long.csv"
TLC_HEADER_ROW = 4
TLC_VALUE_ROW = 5
TLC_START_COL = 3
...
def extract_freight_rows(
    ...
    if min_row == max_row:
        ...
        supplier = clean_value(value_worksheet.cell(value_row_idx, 2).value)
    elif max_row == min_row + 1:
        header_row_idx = min_row
        value_row_idx = max_row
        data_start_col = min_col
        metric_label_source_row = 17
        supplier = clean_value(value_worksheet.cell(19, 2).value)
    else:
        raise ValueError(f"Expected a one-row or two-row freight range, got {range_ref}.")

    section_name = clean_value(value_worksheet.cell(17, 2).value) or "FRETE INTERNACIONAL"
    metric_detail = clean_value(value_worksheet.cell(18, 2).value)
    rows: list[dict[str, Any]] = []
...
def find_last_nonempty_col(worksheet: Any, row_idx: int, start_col: int) -> int:
    last_col = start_col - 1
    for col_idx in range(start_col, worksheet.max_column + 1):
        if clean_value(worksheet.cell(row_idx, col_idx).value) is not None:
            last_col = col_idx

    if last_col < start_col:
        raise ValueError(f"No data found in row {row_idx} from column {start_col}.")
...
def extract_tlc_rows(
    ...
    rows: list[dict[str, Any]] = []
    max_col = find_last_nonempty_col(value_worksheet, TLC_VALUE_ROW, TLC_START_COL)
    source_range = f"{get_column_letter(TLC_START_COL)}{TLC_VALUE_ROW}:{get_column_letter(max_col)}{TLC_VALUE_ROW}"
    source_label = clean_value(value_worksheet.cell(TLC_VALUE_ROW, 2).value)

    for col_idx in range(TLC_START_COL, max_col + 1):
        value = clean_value(value_worksheet.cell(TLC_VALUE_ROW, col_idx).value)
        if value is None:
            continue

        column_letter = get_column_letter(col_idx)
        price_period = clean_value(value_worksheet.cell(TLC_HEADER_ROW, col_idx).value)
        rows.append(
            {
                "source_file": str(source_file),
                "source_sheet": sheet_name,
                "source_range": source_range,
                "source_cell": f"{column_letter}{TLC_VALUE_ROW}",
                "metric_row": TLC_VALUE_ROW,
                "metric_label_source_row": TLC_VALUE_ROW,
                "period_header_source_row": TLC_HEADER_ROW,
                ...
                **period_fields(price_period),
                "value": value,
                "formula": formula_worksheet.cell(TLC_VALUE_ROW, col_idx).value,
            }
        )
...
def extract_long_rows(
    ...
    for range_ref in range_refs:
        if range_ref.upper() == "B2:N8":
            rows.extend(
                extract_update_index_rows(
                    value_worksheet,
                    formula_worksheet,
                    source_file,
                    sheet_name,
        ...
        elif range_ref.upper() in {"B19:N19", "E21:O22"}:
            rows.extend(
            ...
            raise ValueError(f"Unsupported Cristalpet range: {range_ref}")
...
def default_excel_output_path(source_file: Path, output_dir: Path) -> Path:
    ...
    return output_dir / f"{safe_stem}_indices_with_tlc_final.xlsx"
...
def write_excel_output(
    ...
    write_rows_to_sheet(workbook, "raw_B2_N8_E21_O22", raw_rows)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Cristalpet indices extraction pipeline."
    )
    parser.add_argument(
        "--file",
        type=Path,
        default=None,
        help="Specific workbook to process. If omitted, the latest Cristalpet workbook in data is used.",
    )
...
def main() -> None:
    ...
    try:
        ...
        tlc_rows = extract_tlc_rows(
            tlc_value_worksheet,
            tlc_formula_worksheet,
            source_file,
            tlc_sheet_name,
        )
        final_rows.extend(tlc_rows)
        ...
        print(f"tlc_source_row={TLC_VALUE_ROW}")
