...
DEFAULT_OUTPUT_FILE = Path("data") / "archive" / "legacy_output" / "04. Engepack_indices_final.xlsx"
DEFAULT_MAPPING_FILE = Path("data") / "reference" / "mappings" / "Mapping_Columns.xlsx"
DEFAULT_MAPPING_SHEET = "04. Engepack"
...
LOOKUP_ALIASES = {
    "Total Landing Cost $": "Total Landing Cost",
}
...
NORMALIZED_LOOKUP_ALIASES = {
    normalize_key(source): normalize_key(target)
    for source, target in LOOKUP_ALIASES.items()
}
...
def mapped_values_for_key(
    mapping: dict[str, dict[str, Any]],
    normalized_key: str,
) -> tuple[dict[str, Any] | None, bool]:
    exact_match = mapping.get(normalized_key)
    if exact_match is not None:
        return exact_match, False

    alias_key = NORMALIZED_LOOKUP_ALIASES.get(normalized_key)
    if alias_key is None:
        return None, False

    return mapping.get(alias_key), True
...
def apply_mapping(
    ...
    try:
        ...
        for row_idx in range(2, worksheet.max_row + 1):
            ...
            mapping_values, used_alias = mapped_values_for_key(mapping, normalized)
            updated += 1
            ...
            if lookup_value is None or str(lookup_value).strip() == "":
                ...
                matched += 1
                if used_alias:
                    alias_matched[str(lookup_value)] = (
                        alias_matched.get(str(lookup_value), 0) + 1
                    )
        ...
        alias_rows = [
            {
                lookup_column_name: key,
                "row_count": count,
                "alias_used": LOOKUP_ALIASES.get(key),
            }
            for key, count in sorted(alias_matched.items())
        ]
        metadata_rows = [
            ...
            {"field": "lookup_aliases", "value": str(LOOKUP_ALIASES)},
            {"field": "updated_rows", "value": updated},
            {"field": "matched_rows", "value": matched},
            {"field": "alias_matched_rows", "value": sum(alias_matched.values())},
        ...
        write_rows_to_sheet(
            ...
            "alias_mapping_used",
            alias_rows,
            [lookup_column_name, "row_count", "alias_used"],
        )
        write_rows_to_sheet(
            workbook,
        ...
        return {
            ...
            "alias_matched_rows": sum(alias_matched.values()),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Add Mapping Columns and calculation flags to the Engepack final_data sheet."
        )
    )
